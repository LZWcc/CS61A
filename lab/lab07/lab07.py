...
class FreeChecking(Account):
    ...
    def withdraw(self, amount):
        ...
        if amount > self.balance:
            return "Insufficient funds"
        self.balance = self.balance - amount
        if FreeChecking.free_withdrawals <= 0 or self.cnt > 0:
            if self.balance - FreeChecking.withdraw_fee >= 0:
...
def without(s, i):
    ...
    if s is Link.empty:
        ...
        return without(s.rest, i - 1) # 跳过当前节点
    else:
...
def duplicate_link(s, val):
    ...
    """
    if s is Link.empty:
        return # 函数目的是原地修改而不是创建和返回一个新的链表。这种函数通常返回 None（即不显式返回值）
    # 情况1：当前节点值等于目标值
    elif s.first == val:
        # 第1步：记住原始的后续链表
        remaining = s.rest
