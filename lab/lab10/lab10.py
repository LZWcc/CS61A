...
def calc_eval(exp):
    ...
    if isinstance(exp, Pair):
        operator = exp.first # Q2更新：例如 (+ 1 2)，+ 是运算符
        operands = exp.rest # Q2更新：例如 (+ 1 2)，1 和 2 是操作数
        if operator == 'and': # and 表达式
            return eval_and(operands)
        elif operator == 'define': # define 表达式
            return eval_define(operands)
        else: # Call expressions
            return calc_apply(calc_eval(operator), exp.rest) # Q2更新：operator 的类型是什么？
    elif exp in OPERATORS:   # 查找过程
        return OPERATORS[exp]
    elif isinstance(exp, int) or isinstance(exp, bool):   # 数字和布尔值
        return exp
    elif exp in bindings: # Q4修改条件：变量存储在哪里？
        return bindings[exp] # Q4更新：如何访问变量？

def calc_apply(op, args):
    return op(args) # 可知op是一个函数

def floor_div(args): # 传入的args是一个Pair链表, 第一个数是被除数, 后继数字都是除数
    """
    ...
    while current is not nil:
        divisor = calc_eval(current.first)
